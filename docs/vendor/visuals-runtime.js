...
(function (globalScope) {
  ...
  function resolveScriptable(value, context, fallback) {
  ...
  function drawPointShape(ctx, point, radius, style) {
    const pointStyle = String(style || 'circle');
    ctx.beginPath();
    ...
    if (pointStyle === 'rectRounded') {
      drawRoundedRect(ctx, point.x - radius, point.y - radius, radius * 2, radius * 2, Math.max(1, radius * 0.35));
      return;
    }
  ...
  class TinyChart {
    ...
    handleCanvasClick(nativeEvent) {
      ...
      let nearest = null;
      this.datasetMeta.forEach((meta, datasetIndex) => {
        (meta?.data || []).forEach((element, index) => {
          if (!element || !isFiniteNumber(element.x) || !isFiniteNumber(element.y)) return;
          const distance = Math.hypot(Number(element.x) - pointer.x, Number(element.y) - pointer.y);
          if (!nearest || distance < nearest.distance) {
            nearest = { datasetIndex, index, element, distance };
          }
        });
      });
      const elements = nearest && nearest.distance <= 28
        ? [{ datasetIndex: nearest.datasetIndex, index: nearest.index, element: nearest.element }]
        : [];
      onClick(pointer, elements, this);
    }
    ...
    drawLine() {
      ...
      sortedDatasetEntries.forEach(({ dataset, datasetIndex }) => {
        ...
        segments.forEach((segment) => {
          if (!segment.length) return;
          const borderDash = dataset.segment?.borderDash;
          if (typeof borderDash === 'function' && segment.length > 1) {
            for (let index = 1; index < segment.length; index += 1) {
              const previous = segment[index - 1];
              const current = segment[index];
              const dash = resolveScriptable(borderDash, {
                p0DataIndex: previous.index,
                p1DataIndex: current.index,
                datasetIndex,
              }, []);
        ...
        points.forEach((point) => {
          if (!point) return;
          const context = { dataIndex: point.index, datasetIndex, raw: values[point.index] };
          const pointRadius = Number(resolveScriptable(dataset.pointRadius, context, 0));
          if (!isFiniteNumber(pointRadius) || pointRadius <= 0) return;
          const pointStyle = resolveScriptable(dataset.pointStyle, context, 'circle');
          const fillColor = resolveScriptable(dataset.pointBackgroundColor, context, dataset.backgroundColor || lineColor);
          const borderColor = resolveScriptable(dataset.pointBorderColor, context, lineColor);
          drawPointShape(ctx, point, pointRadius, pointStyle);
          ctx.fillStyle = alphaColor(fillColor, deEmphasize ? 0.35 : 1);
          ctx.fill();
          ctx.lineWidth = 1.5;
          ctx.strokeStyle = alphaColor(borderColor, deEmphasize ? 0.45 : 1);
          ctx.stroke();
        });
